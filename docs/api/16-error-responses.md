...
```json
{
  ...
  "status": {
    ...
    "errorCode": "ERR_COMPANY_ACCESS_DENIED"
  }
}
```
...
```json
{
  ...
  "status": {
    ...
    "errorCode": "ERR_MIN_SERVICE_NOT_MET"
  }
}
```
    ...
    if (!response.ok) {
      ...
      switch (data.status.statusCode) {
        ...
        case 500:
          ...
          showError(`An error occurred. Reference: ${data.status.correlationId || 'N/A'}`);
          break;
        default:
          showError(data.status.message);
      }
